"""PaskitScene - ManimGL Scene wrapper for PyQt6 integration.
...
class PaskitScene(Scene):
    ...
    def setup(self):
        ...
        logger.info("[PaskitScene.setup] Initializing axes...")
        ...
        logger.info(f"[PaskitScene.setup] Added axes, mobjects: {len(self.mobjects)}")
    ...
    def play(self, *animations, **kwargs):
        ...
        logger.info(f"[PaskitScene.play] Capturing {len(animations)} animations, run_time={run_time}")
        ...
        for animation in animations:
            ...
            if mobject is not None and mobject not in self.mobjects:
                self.add(mobject)
                logger.info(f"[PaskitScene.play] Added {type(mobject).__name__} to scene")
            elif mobject is not None:
                logger.debug(f"[PaskitScene.play] {type(mobject).__name__} already in scene")
            ...
            if mobject is not None and hasattr(mobject, 'get_points'):
                try:
                    ...
                    logger.warning(f"[PaskitScene.play] Failed to store starting points: {e}")

            # Initialize animation
            try:
                if hasattr(animation, 'begin'):
                    animation.begin()
                    logger.debug(f"[PaskitScene.play] Called begin() on {type(animation).__name__}")
                    ...
                    if hasattr(animation, 'interpolate'):
                        ...
                        logger.debug(f"[PaskitScene.play] Called interpolate(1.0) to restore mobject state")

            except Exception as e:
                logger.error(f"[PaskitScene.play] Failed to initialize animation: {e}")

            # Check again after begin() - some animations might add mobjects differently
            if mobject is not None and mobject not in self.mobjects:
                self.add(mobject)
                logger.info(f"[PaskitScene.play] Added {type(mobject).__name__} to scene after begin()")
        ...
        if not self.timeline.is_playing and len(self.timeline.segments) > 0:
            logger.info("[PaskitScene.play] Auto-starting playback")
            self.timeline.play()

        logger.info(f"[PaskitScene.play] Timeline now has {len(self.timeline.segments)} segments, "
    ...
    def wait(self, duration: float = 1.0):
        ...
        """
        logger.info(f"[PaskitScene.wait] Adding wait/physics segment: {duration}s")
        ...
        for mob in self.mobjects:
            ...
            if updaters:
                logger.info(f"[PaskitScene.wait] Found {len(updaters)} updaters on {type(mob).__name__}")
        ...
        if physics_count == 0:
            ...
            logger.info(f"[PaskitScene.wait] Created {physics_count} physics segments")

        # Auto-play if not already playing
        if not self.timeline.is_playing:
            logger.info("[PaskitScene.wait] Auto-starting playback")
            self.timeline.play()
    ...
    def seek_to_time(self, time: float, snap: bool = True):
        ...
        """
        logger.info(f"[PaskitScene.seek_to_time] Seeking to {time:.2f}s (snap={snap})")
    ...
    def update(self, dt: float):
        ...
        if hasattr(self, '_last_state_log'):
            if self.timeline.current_time - self._last_state_log > 2.0:
                logger.debug(f"[PaskitScene.update] t={self.timeline.current_time:.2f}s, "
    ...
    def _update_mobjects(self, dt: float):
        ...
        if updater_count > 0 and int(self.timeline.current_time * 2) != int((self.timeline.current_time - dt) * 2):
            logger.debug(f"[PaskitScene] Called {updater_count} updaters at t={self.timeline.current_time:.2f}s")
